...
if (buttonUpdateItem.length) {
    buttonUpdateItem.forEach(button => {
        ...
        async function updateEmployeeModal() {
            ...
            // const selectDish=document.querySelector("#dish");
            const modalTitle=document.querySelector('.modalTitle');
            try {
                const response = await fetch(`/admin/api/v1/order-slips/detail/update/${id}`);
                if (!response.ok) throw new Error('Failed to fetch employee data');

                let detailSlip = await response.json(); // Dữ liệu nhân viên từ server

                clearForm(document.querySelector("[form-create-update]"));

                // Điền dữ liệu vào form
                document.querySelector('#dish').value = detailSlip.MAMON;
                document.querySelector('#quantity').value = detailSlip.SOLUONG;
                document.querySelector('#orderSlipId').value = detailSlip.MAPHIEUDATMON;

                // selectDish.setAttribute("disabled", true); // Thêm thuộc tính 'disabled'
                ...
                modalTitle.textContent = 'Cập nhật chi tiết đặt món';
                ...
                if (buttonCreate) {
                    ...
                    buttonCreate.forEach(button => {
                        button.addEventListener('click', () => {
                            const action = path + `/update/${id}?_method=PATCH`;
