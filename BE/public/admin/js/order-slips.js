...
if (buttonUpdateItem.length) {
    buttonUpdateItem.forEach(button => {
        ...
        async function updateEmployeeModal() {
            ...
            try {
                const response = await fetch(`/admin/api/v1/order-slips/update/${id}`);
                if (!response.ok) throw new Error('Failed to fetch employee data');

                let order = await response.json(); // Dữ liệu nhân viên từ server

                clearForm(document.querySelector("[form-create-update]"));

                // Điền dữ liệu vào form
                document.querySelector('#IDKhachHang').value = order.MAKHACHHANG;
                document.querySelector('#SoBan').value = order.SOBAN;
                ...
                document.querySelector('.exception1').style.display = 'none';
