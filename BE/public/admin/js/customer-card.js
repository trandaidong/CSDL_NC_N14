    ...
    if (buttonUpdateItem.length) {
        buttonUpdateItem.forEach(button => {
            ...
            async function updateEmployeeModal() {
                ...
                try {
                    const response = await fetch(`/admin/api/v1/customer-cards/update/${id}`);
                    if (!response.ok) throw new Error('Failed to fetch employee data');
                    let card = await response.json(); // Dữ liệu nhân viên từ server

                    clearForm(document.querySelector("[form-create-update]"));

                    // Điền dữ liệu vào form
                    document.querySelector('#MaKhachHang').value = card.MAKHACHHANG;
                    document.querySelector('#typeCard').value = card.LOAITHE;
                    ...
                    modalTitle.textContent = 'Cập nhật thẻ khách hàng';
                    ...
                    if (buttonCreate) {
                        ...
                        buttonCreate.forEach(button => {
                            button.addEventListener('click', () => {
                                const action = path + `/update/${card.MATHE}?_method=PATCH`;
